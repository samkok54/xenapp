<template>
  <div>
    ...
    margin-right: 1vw; 
    height:100%;">
      <h1 class="ui dividing header Kanitonly">Provision {{customer_name}}</h1>
...
<div class="ui grid">
  <div class="seven wide column" >
        ...
        <div class="field Kanitonly" v-for="(item,index) in getservicedesktop" style="text-align: left; ">
          <div v-if="item['enabled'] == 'False'" class="ui checkbox">
...
export default {
  ...
  methods: {
    getservice () {
      this.$http.post(process.env.IPFLASK + '/Getservicecus', {cus: this.customer_name}).then((response) => {
    ...
    submit () {
      ...
      this.$http.post(process.env.IPFLASK + '/provisionappcus', {cus: this.customer_name, app: this.sendapp}).then((response) => {
